...
### 5. Строка с версией
на экране медитаций после редизайна строка с версией не поместилась в экран на устройстве iPhoneSE
**Контроль:** обновил стиль footerText, установил marginTop: 'auto'

## Проблемы с версиями

В процессе разработки столкнулся с типичными проблемами React Native экосистемы:

1. **Несовместимость Expo Go и SDK** — пришлось явно указывать версию шаблона (`--template blank@sdk-54`) и следить за версией Expo Go на телефоне.

2. **Конфликт React 19 и Expo SDK 54** — использовал React 18.2.0 и `@types/react@18.2.0` с флагом `--legacy-peer-deps`.

3. **Проблемы с Babel плагинами** — временно отключил `react-native-reanimated/plugin`, чтобы избежать ошибок сборки.

4. **Сборка пакетов на Linux** — использовал `--ignore-scripts` при установке, чтобы пропустить пост-установочные скрипты, которые не работали в моём окружении.

**Итог:** разработка под React Native на Linux требует внимания к версиям и готовности к обходу проблем с зависимостями.
